"""
Step 2 (CN, extended): Firm betas + AR/CAR + AR-long for QR & EC.

This is your previous CN Step 2 with one main addition:
 - It now also produces an EU-style AR-long file:
       step2_event_AR_long_cn.csv
   with columns:
       Ticker, EventType, Source, EventDate, EventDate_adj,
       k, Date, AR, AVol, Bundled_Flag

AR-long spans a symmetric range around each event date
(default: k = -10, ..., +10). This file can be used to run
dynamic (staggered) event-time regressions similar to the EU part.

CAR windows (CN now aligned with EU):
...
OUT_ARLONG   = ROOT / "step2_event_AR_long_cn.csv"
...
AFTER_CLOSE_SHIFT = False  # if True: EventDate_adj = next trading day
...
def collapse_ec_sentences(ec_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Build order key: QuestionRank * 10 + {1->0 (Q), 2->1 (A), else 9}
...
def process_events(events_df: pd.DataFrame, event_type_label: str, panel: pd.DataFrame):
    betas_rows, arcar_rows, arlong_rows = [], [], []
    ...
    for tk, evs in events_df.groupby("Ticker"):
        ...
        for _, e in evs.sort_values("EventDate").iterrows():
            ...
            exp_all = (
                bd["const"]
                + bd["Mkt-RF"] * F_all["Mkt-RF"].to_numpy(dtype=np.float64)
                + bd["SMB"]    * F_all["SMB"].to_numpy(dtype=np.float64)
                + bd["HML"]    * F_all["HML"].to_numpy(dtype=np.float64)
                + bd["RMW"]    * F_all["RMW"].to_numpy(dtype=np.float64)
                + bd["CMA"]    * F_all["CMA"].to_numpy(dtype=np.float64)
            )
            ar_all = y_all - exp_all
            ...
            event_adj = ev_dt
            if AFTER_CLOSE_SHIFT:
                # use next trading day if exists
                if ev_ix + 1 < len(sub):
                    event_adj = sub.loc[ev_ix + 1, DATE_COL_PANEL]

            for k in range(-ARLONG_K_PRE, ARLONG_K_POST + 1):
                t_idx = ev_ix + k
                if t_idx < 0 or t_idx >= len(sub):
                    continue

                ar_val = float(ar_all[t_idx])
                date_t = sub.loc[t_idx, DATE_COL_PANEL]
    ...
    return (
        pd.DataFrame(betas_rows),
        pd.DataFrame(arcar_rows),
        pd.DataFrame(arlong_rows),
    )
...
betas_qr, arcar_qr, arlong_qr = process_events(qr_events, "QR", panel)
betas_ec, arcar_ec, arlong_ec = process_events(ec_events, "EC", panel)
...
ar_long_all = pd.concat([arlong_qr, arlong_ec], ignore_index=True)
ar_long_all = ar_long_all.sort_values(["Ticker", "EventDate", "k", "Date"])
ar_long_all.to_csv(OUT_ARLONG, index=False)
...
print(f" AR-long: {len(ar_long_all):,} rows")
print(f"Saved:\n - {OUT_BETAS}\n - {OUT_ARCAR}\n - {OUT_EVENTS}\n - {OUT_ARLONG}")
